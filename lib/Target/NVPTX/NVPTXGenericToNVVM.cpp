...
Value *GenericToNVVM::getOrInsertCVTA(Module *M, Function *F,
  ...
  EVT ExtendedGVType = EVT::getEVT(GVType->getPointerElementType(), true);
  if (!ExtendedGVType.isInteger() && !ExtendedGVType.isFloatingPoint()) {
    ...
    // required.
    DestTy =
        PointerType::get(GVType->getPointerElementType(), llvm::ADDRESS_SPACE_GENERIC);
    CVTA = Builder.CreateBitCast(CVTA, DestTy, "cvta");
  } else {
    // A simple CVTA is enough.
    SmallVector<Type *, 2> ParamTypes;
    ParamTypes.push_back(PointerType::get(GVType->getPointerElementType(),
